...
- **Dead code or commented-out blocks left in place.** Phaser is the sole rendering engine; the old React Flow path (`?engine=reactflow`, `@xyflow/react`) was removed, so flag any reintroduced React Flow / `@xyflow` code as dead. Do flag commented-out logic blocks, unused imports, and references to features that no longer exist.
